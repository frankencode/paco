#ifndef PTE_CUSTOMLIST_HPP
#define PTE_CUSTOMLIST_HPP

#include <new>
#include <ftl/Array.hpp>
#include "UseFtl.hpp"
#include "CustomListObserver.hpp"

namespace pte
{

FTL_EXCEPTION(ListException, Exception);

template<class T>
class CustomList: public Sequence<T>, public NonCopyable, public virtual BackRefList
{
public:
	CustomList();
	~CustomList();
	...
	typedef CustomListObserver<T> Observer;
	inline void setObserver(Ref<Observer> observer) { observer_ = observer; }
	
	void clear();
	
	inline bool has(int i) const { return ((0 <= i) && (i < length_)) || ((-length_ <= i) && (i < 0)); }
	T get(int i) const;
	void set(int i, T e);
	...
	/*bool operator<(const CustomList& b) const;
	bool operator==(const CustomList& b) const;
	inline bool operator>(const CustomList& b) const { return b < *this; }
	inline bool operator!=(const CustomList& b) const { return !(*this == b); }
	inline bool operator<=(const CustomList& b) const { return (*this < b) || (*this == b); }
	inline bool operator>=(const CustomList& b) const { return (b < *this) || (*this == b); }
	*/
	/*
	void sort();
	void unique(Ref< CustomList<int> > frq = 0);
	void statistic(Ref< CustomList<T> > members, Ref< CustomList<int> > frq);
	*/
	
	void enableIndexCaching() { if (!posCached_) posCached_ = new Pos; }
	
protected:
	CustomList(Ref<CustomList> parent, int index0, int length);
	virtual Ref<CustomList, Owner> newList() const { return new CustomList; }
	virtual Ref<CustomList, Owner> newChildList(Ref<CustomList> parent, int i, int n) const { return new CustomList(parent, i, n); }
...
private:
	...
	inline Ref<CustomList, Owner> range(int i, int n) const { return newChildList(this, i, n); }
	inline Ref<CustomList, Owner> copy(int i, int n) const;
	
	// --
	
	int find(int i, int n, const T& e) const;
	void push(int i, int n, CustomList* b);
	int find(int i, int n, CustomList* b) const;
	...
	Ref< CustomList<T>, Owner > parent_;
	int index0_;
	int length_;
...
template<class T>
CustomList<T>::CustomList(Ref<CustomList> parent, int index0, int length)
	: parent_(parent),
	  index0_(index0),
	  length_(length),
	  front_(0),
...
template<class T>
void CustomList<T>::push(int i, int n, const T* v)
{
	if (n == 0) return;
	
	if (i < 0) i += length_ + 1;
	if (!((0 <= i) && (i <= length_)))
		FTL_THROW(ListException, "Wrong index");
...
template<class T>
void CustomList<T>::pop(int i, int n, T* v)
{
	if (n == 0) return;
...
template<class T>
void CustomList<T>::read(int i, int n, T* v) const
{
	if (n == 0) return;
...
template<class T>
void CustomList<T>::write(int i, int n, const T* v)
{
	if (n == 0) return;
...
template<class T>
void CustomList<T>::fill(int i, int n, const T& e)
{
	Array<T> v(n);
	v.clear(e);
	write(i, n, v.data());
...
template<class T>
inline T CustomList<T>::get(int i) const
{
	if (i < 0) i += length_;
	if (!((0 <= i) && (i < length_)))
		FTL_THROW(ListException, "Wrong index");
...
template<class T>
inline void CustomList<T>::set(int i, T e)
{
	if (i < 0) i += length_;
	if (!((0 <= i) && (i < length_)))
		FTL_THROW(ListException, "Wrong index");
...
template<class T>
Ref<CustomList<T>, Owner> CustomList<T>::copy(int i, int n) const
{
	if (i < 0) i += length_;
	if (!((0 <= i) && (i + n <= length_)))
		FTL_THROW(ListException, "Wrong index or length");
	
	Ref<CustomList, Owner> b;
...
template<class T>
int CustomList<T>::find(int i, int n, const T& e) const
{
...
template<class T>
int CustomList<T>::replace(const T& a, const T& b)
{
	int i = 0;
	int n = 0;
	while (i < length_) {
...
template<class T>
void CustomList<T>::push(int i, int n, CustomList* b)
{
	if (n == 0) return;
...
template<class T>
int CustomList<T>::find(int i, int n, CustomList* b) const
{
	if (length_ == 0) return 0;
...
/*template<class T>
bool CustomList<T>::operator<(const CustomList& b_) const
{
	const CustomList* a = this;
	int i = 0, n = length_;
	while (a->parent_) {
		i += a->index0_;
		a = a->parent_;
